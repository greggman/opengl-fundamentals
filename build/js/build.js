...
var Builder = function(outBaseDir, options) {
  ...
  var applyTemplateToContent = function(templatePath, contentFileName, outFileName, opt_extra, data) {
    ...
    metaData['url'] = "http://openglfundamentals.org/" + relativeOutName;
    metaData['screenshot'] = "http://openglfundamentals.org/opengl/lessons/resources/openglfundamentals.jpg";
    var basename = path.basename(contentFileName, ".md");
    [".jpg", ".png"].forEach(function(ext) {
      var filename = path.join("opengl", "lessons", "screenshots", basename + ext);
      if (fs.existsSync(filename)) {
        metaData['screenshot'] = "http://openglfundamentals.org/opengl/lessons/screenshots/" + basename + ext;
      }
    });
  ...
  var addArticleByLang = function(article, lang) {
    ...
    var url = "http://openglfundamentals.org" + article.dst_file_name;
    if (!articleInfo) {
      articleInfo = {
        url: url,
        changefreq: 'monthly',
  ...
  var getLanguageSelection = function(lang) {
    var lessons = lang.lessons || ("opengl/lessons/" + lang.lang);
  ...
  this.process = function(options) {
    ...
    options.lessons     = options.lessons     || ("opengl/lessons/" + options.lang);
    options.toc         = options.toc         || ("opengl/lessons/" + options.lang + "/toc.html");
    options.template    = options.template    || "build/templates/lesson.template";
    options.examplePath = options.examplePath === undefined ? "/opengl/lessons/" : options.examplePath;

    g_articles = [];
    g_langInfo = hanson.parse(fs.readFileSync(path.join(options.lessons, "langinfo.hanson"), {encoding: "utf8"}));

    applyTemplateToFiles(options.template, path.join(options.lessons, "opengl*.md"), options);
    ...
    return tasks.reduce(function(cur, next){
      ...
      var feed = new Feed({
        ...
        image:          'http://openglfundamentals.org/opengl/lessons/resources/openglfundamentals.jpg',
        ...
        author: {
          ...
          link:       'http://openglfundamentals.org/contributors.html',
        },
      });
      ...
      articles.forEach(function(article, ndx) {
        feed.addItem({
          title:          article.title,
          link:           "http://openglfundamentals.org" + article.dst_file_name,
          description:    "",
          author: [
            {
              name:       'OpenGLFundamenals Contributors',
              link:       'http://openglfundamentals.org/contributors.html',
            },
          ],
          // contributor: [
  ...
  this.writeGlobalFiles = function() {
    var sm = sitemap.createSitemap ({
      hostname: 'http://openglfundamentals.org',
      cacheTime: 600000,
    });
    var articleLangs = { };
    ...
    copyFile(path.join(g_outBaseDir, "opengl/lessons/atom.xml"), path.join(g_outBaseDir, "atom.xml"));
    copyFile(path.join(g_outBaseDir, "opengl/lessons/index.html"), path.join(g_outBaseDir, "index.html"));
...
var b = new Builder("out", {
  origPath: "opengl/lessons",  // english articles
});
...
var readdirs = function(dirpath) {
  ...
  return fs.readdirSync("opengl/lessons")
      .map(addPath)
      .filter(dirsOnly);
};
...
var langs = [
  // English is special (sorry it's where I started)
  {
    ...
    lessons: "opengl/lessons",
    lang: 'en',
    toc: 'opengl/lessons/toc.html',
    examplePath: '/opengl/lessons/',
  },
];

langs = langs.concat(readdirs("opengl/lessons")
    .filter(isLangFolder)
    .map(pathToLang));
